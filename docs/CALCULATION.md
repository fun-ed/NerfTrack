...
GPT-6 cache writes are priced at 1.25x the input rate. Prompts over 272K
input tokens use 2x input/cache rates and 1.5x output; Fast mode is 2x the
applicable rates.
...
estimates are invalidated. On startup NerfTrack compares the pricing digest and pricing-rule
version, estimator versions, and installed-bundle marker with the last completed rebuild. When
one changes, it reparses every discoverable source JSONL rollout, applies historical tier corrections,
reprices token events, and rebuilds measurements, estimates, windows, and graph points in one
